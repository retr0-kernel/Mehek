...
func (mq *MenuQuery) QueryBrand() *BrandQuery {
	query := (&BrandClient{config: mq.config}).Query()
...
func (mq *MenuQuery) QueryItems() *MenuItemQuery {
...
func (mq *MenuQuery) WithBrand(opts ...func(*BrandQuery)) *MenuQuery {
	query := (&BrandClient{config: mq.config}).Query()
...
func (mq *MenuQuery) WithItems(opts ...func(*MenuItemQuery)) *MenuQuery {
	query := (&MenuItemClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MenuQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Menu, error) {
	...
	if query := mq.withBrand; query != nil {
		if err := mq.loadBrand(ctx, query, nodes, nil,
			func(n *Menu, e *Brand) { n.Edges.Brand = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withItems; query != nil {
		if err := mq.loadItems(ctx, query, nodes,
...
func (mq *MenuQuery) loadBrand(ctx context.Context, query *BrandQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *Brand)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Menu)
	for i := range nodes {
		if nodes[i].brand_menus == nil {
			continue
		}
		fk := *nodes[i].brand_menus
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "brand_menus" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MenuQuery) loadItems(ctx context.Context, query *MenuItemQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *MenuItem)) error {
	...
	for _, n := range neighbors {
		fk := n.menu_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "menu_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "menu_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
