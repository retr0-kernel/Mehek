...
func (iq *IngredientQuery) QueryMenuItems() *MenuItemQuery {
	query := (&MenuItemClient{config: iq.config}).Query()
...
func (iq *IngredientQuery) QueryInventoryItems() *InventoryItemQuery {
...
func (iq *IngredientQuery) Clone() *IngredientQuery {
	...
	return &IngredientQuery{
		...
		withMenuItems:      iq.withMenuItems.Clone(),
		withInventoryItems: iq.withInventoryItems.Clone(),
...
func (iq *IngredientQuery) WithMenuItems(opts ...func(*MenuItemQuery)) *IngredientQuery {
	query := (&MenuItemClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withMenuItems = query
	return iq
}
...
func (iq *IngredientQuery) WithInventoryItems(opts ...func(*InventoryItemQuery)) *IngredientQuery {
	query := (&InventoryItemClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withInventoryItems = query
	return iq
}
...
func (iq *IngredientQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ingredient, error) {
	var (
		...
		_spec       = iq.querySpec()
		loadedTypes = [2]bool{
			iq.withMenuItems != nil,
			iq.withInventoryItems != nil,
		}
	)
	...
	if query := iq.withMenuItems; query != nil {
		if err := iq.loadMenuItems(ctx, query, nodes,
			func(n *Ingredient) { n.Edges.MenuItems = []*MenuItem{} },
			func(n *Ingredient, e *MenuItem) { n.Edges.MenuItems = append(n.Edges.MenuItems, e) }); err != nil {
			return nil, err
		}
	}
	if query := iq.withInventoryItems; query != nil {
		if err := iq.loadInventoryItems(ctx, query, nodes,
			func(n *Ingredient) { n.Edges.InventoryItems = []*InventoryItem{} },
			func(n *Ingredient, e *InventoryItem) { n.Edges.InventoryItems = append(n.Edges.InventoryItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *IngredientQuery) loadMenuItems(ctx context.Context, query *MenuItemQuery, nodes []*Ingredient, init func(*Ingredient), assign func(*Ingredient, *MenuItem)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(ingredient.MenuItemsTable)
		s.Join(joinT).On(s.C(menuitem.FieldID), joinT.C(ingredient.MenuItemsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(ingredient.MenuItemsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(ingredient.MenuItemsPrimaryKey[1]))
...
func (iq *IngredientQuery) loadInventoryItems(ctx context.Context, query *InventoryItemQuery, nodes []*Ingredient, init func(*Ingredient), assign func(*Ingredient, *InventoryItem)) error {
	...
	query.Where(predicate.InventoryItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(ingredient.InventoryItemsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.inventory_item_ingredient
		if fk == nil {
			return fmt.Errorf(`foreign-key "inventory_item_ingredient" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "inventory_item_ingredient" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
