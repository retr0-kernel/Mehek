...
func (oiq *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	...
	if oiq.withOrder != nil || oiq.withMenuItem != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oiq.withMenuItem; query != nil {
		if err := oiq.loadMenuItem(ctx, query, nodes, nil,
...
func (oiq *OrderItemQuery) loadMenuItem(ctx context.Context, query *MenuItemQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *MenuItem)) error {
	...
	nodeids := make(map[int][]*OrderItem)
	for i := range nodes {
		if nodes[i].order_item_menu_item == nil {
			continue
		}
		fk := *nodes[i].order_item_menu_item
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_item_menu_item" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
