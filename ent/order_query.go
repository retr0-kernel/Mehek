...
type OrderQuery struct {
	...
	withResourceAllocations *ResourceAllocationQuery
	withFKs                 bool
...
func (oq *OrderQuery) QueryBrand() *BrandQuery {
	query := (&BrandClient{config: oq.config}).Query()
...
// QueryResourceAllocations chains the current query on the "resource_allocations" edge.
func (oq *OrderQuery) QueryResourceAllocations() *ResourceAllocationQuery {
	query := (&ResourceAllocationClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withResourceAllocations: oq.withResourceAllocations.Clone(),
...
func (oq *OrderQuery) WithBrand(opts ...func(*BrandQuery)) *OrderQuery {
	query := (&BrandClient{config: oq.config}).Query()
...
// WithResourceAllocations tells the query-builder to eager-load the nodes that are connected to
// the "resource_allocations" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithResourceAllocations(opts ...func(*ResourceAllocationQuery)) *OrderQuery {
	query := (&ResourceAllocationClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withResourceAllocations = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	var (
		...
		loadedTypes = [3]bool{
			oq.withBrand != nil,
			oq.withItems != nil,
			oq.withResourceAllocations != nil,
		}
	)
	if oq.withBrand != nil {
		withFKs = true
	...
	if query := oq.withBrand; query != nil {
		if err := oq.loadBrand(ctx, query, nodes, nil,
			func(n *Order, e *Brand) { n.Edges.Brand = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := oq.withResourceAllocations; query != nil {
		if err := oq.loadResourceAllocations(ctx, query, nodes,
			func(n *Order) { n.Edges.ResourceAllocations = []*ResourceAllocation{} },
			func(n *Order, e *ResourceAllocation) {
				n.Edges.ResourceAllocations = append(n.Edges.ResourceAllocations, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (oq *OrderQuery) loadBrand(ctx context.Context, query *BrandQuery, nodes []*Order, init func(*Order), assign func(*Order, *Brand)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Order)
	for i := range nodes {
		if nodes[i].brand_orders == nil {
			continue
		}
		fk := *nodes[i].brand_orders
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "brand_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OrderQuery) loadResourceAllocations(ctx context.Context, query *ResourceAllocationQuery, nodes []*Order, init func(*Order), assign func(*Order, *ResourceAllocation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ResourceAllocation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.ResourceAllocationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.resource_allocation_order
		if fk == nil {
			return fmt.Errorf(`foreign-key "resource_allocation_order" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "resource_allocation_order" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
